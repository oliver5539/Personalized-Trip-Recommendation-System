...
df_att = pd.read_csv(loc_final + 'all_attraction.csv', encoding="utf-8")
...
df_a_u = pd.read_csv(loc_final + 'attraction_ubikev2.csv', encoding="utf-8")
...
def select_mating_pool(pop, fitness, num_parents):
    ...
    # a = pop.shape[1]-1
    parentsf = np.empty((num_parents,1))
    ...
    # parentsf[:] = fitness[roulette]
    parents[:] = pop[roulette]
    return parents, parentsf


# 交配
def crossover(parents, offspring_size, parentsfit, cp):
    # 建立後代陣列
...
for generation in range(num_generations):
    ...
    # 選擇父母
    parents ,parentsfit = select_mating_pool(new_population, fitness, num_parents_mating)
    print_('parent', parents)
    # print(parents)
    # print(len(parents))
    # print(parentsfit)
    # print(len(parentsfit))
    # 交配
    offspring_crossover = crossover(parents, pop_size, parentsfit, c1)
